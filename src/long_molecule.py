...
def create_long_molecule(schema: Schema, rng, barcode, outputbarcode) -> LongMoleculeRecipe:
    ...
    # SWAP CONTAINER.OUT WITH HARCODED PATH
    molecule_fasta = os.path.abspath(f'{container.OUT}/{schema.chrom}.{barcode}.{molnumber}.fa')
    ...
    if schema.mol_coverage < 1:
        ...
        N = max(0, int(rng.normal(schema.mol_coverage, schema.mol_coverage/3)))
        # set ceiling to avoid N being greater than can be sampled
        N = min(N, int(normalized_length/(schema.read_length*2)))
